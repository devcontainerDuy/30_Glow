...
const TableList: React.FC<{ action: React.Dispatch<React.SetStateAction<boolean>>; viewMode: viewMode; dataTable: Array<FileItem> }> = ({
    action,
    viewMode,
    dataTable,
}) => {
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                {dataTable.length > 0 ? (
                    dataTable?.map((item, index) => (
                        <Card
                            key={index}
                            className="border-sidebar-border/70 dark:border-sidebar-border/80 h-full w-full min-w-full gap-0 overflow-hidden rounded-lg py-0"
                        >
                ...
                <TableHeader>
                    <TableRow>
                        <TableHead className="col-span-1 flex items-center">
                            <Checkbox className="cursor-pointer" onClick={() => action((prev) => !prev)} />
                        </TableHead>

...
return new class extends Migration {
    public function up()
    {
        Schema::create('vouchers', function (Blueprint $table) {
            ...
            $table->enum('discount_type', DiscountType::values())->default(DiscountType::FIXED_AMOUNT->value); // Loại giảm giá: percentage (%) hoặc fixed_amount (tiền mặt)
            ...
            $table->integer('per_user_limit')->default(1); // số lần user được phép dùng voucher
            $table->enum('customer_type', VoucherType::values())->default(VoucherType::ALL->value); // Loại khách hàng: ALL (tất cả) hoặc SPECIFIC (cụ thể)
        ...
        Schema::create('voucher_user', function (Blueprint $table) {
            ...
            $table->integer('times_used')->default(0); // Số lần đã sử dụng voucher này

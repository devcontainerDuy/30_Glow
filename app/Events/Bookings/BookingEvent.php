...
class BookingEvent implements ShouldBroadcast
{
    ...
    {
        ($parameters instanceof \Illuminate\Support\Collection) ?
            $this->bookingData = $parameters->transform(function ($item) {
                return $this->transformBooking($item);
            })
            :    $this->bookingData = $this->transformBooking($parameters);
    }

    private function transformBooking($item)
    {
        return [
            'id' => $item->id,
            'user' => $item->user ? [
